...
class RegistrarProfessorForm(forms.Form):
    ...
    def is_valid(self):
        valid = True
        if not super(RegistrarProfessorForm, self).is_valid():
            self.adiciona_erro('Por favor, verifique os dados informados!')
            valid = False

        user_exists = Professor.objects.filter(nome=self.data['nome']).exists()
